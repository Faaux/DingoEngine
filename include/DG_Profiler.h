...
#define RDTSC _rdtsc
	...
	struct DebugEvent
	{
		DebugEvent(u64 clock, u32 threadId, std::string guid, DebugEventType type, std::string name = "", std::string file = "", u32 line = 0, u32 counter = 0)
			: Clock(clock),
			  ThreadId(threadId),
			  GUID(guid),
			  Type(type),
	...
	class DebugProfiler
	{
		...
		friend void PushEvent(DebugEventType type, std::string guid, std::string name, std::string file, u32 line, u32 counter);
	public:
		enum
		{
			FramesRecorded = 300
	...
	inline void PushEvent(DebugEventType type, std::string guid, std::string name = "", std::string file = "", u32 line = 0, u32 counter = 0)
	{
		...
		profiler->_queue.emplace(SDL_GetPerformanceCounter(), threadID, guid, type, name, file, line, counter);
...
#define TIMED_BLOCK__(GUID, Number, Name, ...) timed_block TimedBlock_##Number(GUID, Name, __FILE__, __LINE__, Number, ## __VA_ARGS__)
...
#define TIMED_FUNCTION(...) TIMED_BLOCK_(DEBUG_NAME(__FUNCTION__), __COUNTER__, __FUNCTION__, ## __VA_ARGS__)

#define BEGIN_BLOCK_(GUID, NAME, FILE, LINENUMBER, COUNTER) {PushEvent(Start, GUID, NAME, FILE, LINENUMBER, COUNTER);}
#define END_BLOCK_(GUID) {PushEvent(End, GUID);}
...
#define FRAME_START() {PushEvent(FrameStart, DEBUG_NAME("Frame Start"), "Frame Start", __FILE__, __LINE__, __COUNTER__);}
#define FRAME_END() {PushEvent(FrameEnd, DEBUG_NAME("Frame End"), "Frame End");}
