...
struct DockContext
{
    ...
    void handleDrag(Dock& dock)
    {
        ...
        if (dest_dock)
        {
            if (dockSlots(dock, dest_dock, ImRect(dest_dock->pos, dest_dock->pos + dest_dock->size),
                          false))
            {
                canvas->PopClipRect();
                End();
        ...
        if (dockSlots(dock, nullptr, ImRect(m_workspace_pos, m_workspace_pos + m_workspace_size),
                      true))
        {
            canvas->PopClipRect();
            End();
    ...
    void drawTabbarListButton(Dock& dock)
    {
        ...
        draw_list->AddRectFilled(ImVec2(center.x - 4, min.y + 3), ImVec2(center.x + 4, min.y + 5),
                                 hovered ? color_active : text_color);
        draw_list->AddTriangleFilled(ImVec2(center.x - 4, min.y + 7),
                                     ImVec2(center.x + 4, min.y + 7), ImVec2(center.x, min.y + 12),
                                     hovered ? color_active : text_color);
    }
        ...
        if (BeginChild(tmp, size, true))
        {
            ...
            while (dock_tab)
            {
                ...
                if (dock_tab->active && close_button)
                {
                    ...
                    draw_list->AddLine(center + ImVec2(-3.5f, -3.5f), center + ImVec2(3.5f, 3.5f),
                                       text_color);
                    draw_list->AddLine(center + ImVec2(3.5f, -3.5f), center + ImVec2(-3.5f, 3.5f),
                                       text_color);
                }
                ...
                draw_list->PathBezierCurveTo(pos + ImVec2(-10, size.y), pos + ImVec2(-5, 0),
                                             pos + ImVec2(0, 0), 10);
                ...
                draw_list->PathFillConvex(hovered ? color_hovered
    ...
    bool begin(const char* label, bool* opened, ImGuiWindowFlags extra_flags)
    {
        ...
        if (is_float)
        {
            ...
            bool ret = Begin(
                label, opened, dock.size, -1.0f,
                ImGuiWindowFlags_NoCollapse /*| ImGuiWindowFlags_ShowBorders*/ |
                    extra_flags);  // ImGuiWindowFlags_ShowBorders not used in new version of ImGui
...
void ImGui::SetNextDock(ImGuiDockSlot slot) { g_dock.m_next_dock_slot = slot; }
...
void ImGui::EndDockspace() { EndChild(); }

void ImGui::SetDockActive() { g_dock.setDockActive(); }
...
void ImGui::EndDock() { g_dock.end(); }

void ImGui::DockDebugWindow() { g_dock.debugWindow(); }

void ImGui::SaveDock() { g_dock.saveDock(); }

void ImGui::LoadDock() { g_dock.loadDock(); }
