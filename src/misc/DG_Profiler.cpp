...
DebugProfiler *GetProfiler() { return &profiler; }
...
int ProfilerWork(void *data, FILE *output_stream)
{
    ...
    if (profiler._queue.empty()) return 1;

    DebugEvent event = profiler.GetNextEvent();

    if (profiler._stopRecording) return 0;
    ...
    if (event.Type == FrameStart)
    {
        ...
        {
            while (!pair.second.empty()) pair.second.pop();
        }
    }
    // Handle Frame End
    else if (profiler._hasFrameStart)
    {
        if (event.Type == FrameEnd)
        {
            ...
            // This would avoid unnecessary memory allocations (since the event size per frame will
            // stay realtively constant)
            profiler._concatenatedResultMap.clear();
            profiler._resultMap.clear();
